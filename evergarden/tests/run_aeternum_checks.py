...
parser.add_argument("--aeternum-jar", type=Path)
...
for name in ("evergarden.jar", "advance-magic.jar"):
    module = "evergarden" if name == "evergarden.jar" else "advance-magic"
    shutil.copy2(root / module / "target" / name, plugins / name)
shutil.copy2(args.aeternum_jar or args.source_server / "plugins/AeternumSeasons-4.7.jar", plugins)
