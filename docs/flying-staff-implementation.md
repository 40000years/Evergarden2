# ไม้เท้าบิน: ใช้งาน ติดตั้ง และผลทดสอบ

โค้ดอยู่ใน Advance Magic รุ่น `1.2.15-flying-staff` ใช้คู่กับ Evergarden `3.0.0-e2.11-bedrock-elixir` เมื่อเริ่มใช้ระบบความทนทานต้องอัปเดต JAR ทั้งสองตัว หากใช้ `1.2.15` / `e2.10` อยู่แล้ว การแก้แพ็กน้ำยา Bedrock อัปเดตเฉพาะ Evergarden ได้ แพ็กทั้งสองฝั่งอยู่ใน JAR และ `advance-magic/dist/` ไฟล์แยกใน `advance-magic/art/flying-staff/dist/` เป็นงาน preview ไม่ใช่แพ็กที่เซิร์ฟเวอร์แจก ดูผลตรวจใน [Bedrock item pack audit](bedrock-item-pack-audit.md)

## อัตราและความทนทานปัจจุบัน (1.2.15 / e2.10)

Whale 10%, Hanging Garden 7.5%, Observatory 7.5% ต่อสถานที่ และ Vault 1.5% ต่อการเปิด (หมวดของใช้ 58.5%; ของหายากอื่นคงเดิม) ระบบย้ายค่าเริ่มต้นเรทเก่าใน config ให้ครั้งเดียว โดยเก็บค่าที่ปรับเองไว้ และไม่สุ่มสถานที่ที่บันทึกผลไว้แล้วซ้ำ

ไม้เท้าเริ่มที่ความทนทาน 30 ครั้ง เสกหรือเรียกกลับสำเร็จใช้ 1 ครั้ง เสกไม่สำเร็จ ขี่ ลง และเปิด/ปิด Turbo ไม่หักความทนทาน หมดแล้วเก็บไอเทมไว้ซ่อมก่อนเรียกครั้งใหม่ได้ เที่ยวที่ใช้ครั้งสุดท้ายยังขึ้นขี่และบินได้ตามมานาปกติ

ซ่อมให้เต็มด้วย Wand of Restore ของผู้เล่นอื่น หรือใช้ Core ทำพิธีที่แท่นฟื้นฟูเหมือนคทาเวท เพิ่มความทนทานสูงสุดด้วย Wand Repair Core: เพิ่มครั้งละ 5 และฟื้น 5 ครั้ง สูงสุดระดับ 10 / 80 ครั้ง ไม่รับ Wand Damage Core หรือ Wand Cooldown Core ไม้เท้าเดิมจะได้ค่าความทนทานเมื่ออ่านไอเทมครั้งแรก และไม่รวมเป็นกองเพื่อให้แต่ละอันมีความทนทานของตัวเอง กองเก่าที่มีหลายอันต้องแยกก่อนใช้งาน

รุ่นนี้ build ผ่าน ยังไม่ได้ทดสอบในเกมจริง

## ประวัติการเปลี่ยนแปลง

รุ่น `1.2.14` ถอดสูตรคราฟต์ไม้เท้าบินและถอนสูตรเดิมจากสมุดสูตรตอนเข้าเกม ใช้ร่วมกับ Evergarden `3.0.0-e2.8-flying-staff-loot`: Whale 7.5%, Hanging Garden 5%, Observatory 5% ต่อสถานที่ และ Vault 1% ต่อการเปิด กล่องสำรวจสุ่มครั้งเดียวและได้สูงสุดหนึ่งไม้เท้ารวมทั้งสองกล่อง บันทึกผลไว้ใน chunk จึงไม่สุ่มซ้ำเมื่อโหลดพื้นที่หรือรื้อกล่อง กล่องเก่าที่ยังอยู่จะได้โอกาสสุ่มเพิ่มครั้งเดียวโดยไม่เปลี่ยนของเดิม หากกล่องไม่มีช่องว่างจะไม่เพิ่มของ ตั้งเรทกล่องสำรวจได้ที่ `structures.treasure.flying-staff-chances` ใน config ของ Evergarden ส่วน Vault ลดหมวดของใช้จาก 60% เป็น 59% เพื่อเพิ่มไม้เท้า 1% โดยคงอัตราของหายากอื่นไว้

รุ่น `1.2.13` เพิ่มการเรียกไม้เท้าที่ลืมไว้กลับมา: ถือไอเทมไม้เท้าแล้วคลิกขวาขณะไม่ได้ขี่ จะย้ายยานตัวเดิมมาตรงหน้าและเริ่มแอนิเมชันเรียกใหม่ หากพื้นที่ตรงหน้าติดบล็อกหรือมีผู้ขี่อยู่ จะคงยานไว้ที่เดิม หากยานเดิมถูกถอดออกไปแล้ว ระบบจะสร้างยานใหม่ตามปกติ ไอเทมในกระเป๋าไม่ถูกเพิ่มหรือลด ขณะขี่ยังคงใช้คลิกขวาเปิด/ปิด Turbo

Paper ทดสอบผ่าน 53 checks ในรุ่น `1.2.13` รวมอัตรามานาปกติ 2 ต่อวินาที, Sprint Turbo และ Turbo แบบเปิดค้าง 3 ต่อวินาที, ค่าใช้มานาของการเร่งช่วงสั้น, การเรียกยานตัวเดิมกลับจากระยะไกลโดยไม่เกิดตัวซ้ำ และการคงตำแหน่งเดิมเมื่อจุดเรียกติดบล็อก รุ่น `1.2.14` ยังไม่ได้ทดสอบในเกมจริง

## สถานะการตรวจภาพกระพริบหลังลงจากไม้เท้า

### การแก้ใน 1.2.12 โดยคงระบบฤดูกาล

ไม้เท้ารุ่นก่อนเคลื่อนที่ด้วย `ArmorStand.teleport(..., RETAIN_PASSENGERS)` ทำให้ Paper วาร์ปผู้ขี่และส่ง `PlayerTeleportEvent` ซ้ำในทุก tick ที่ยานเคลื่อนที่ AeternumSeasons รับ event นี้ผ่าน `PlayerArrivalSync` แล้วเรียก `resyncPlayer` เพื่อจัดคิวรีเฟรชพื้นที่รอบผู้เล่น นี่เป็นการเรียกเส้นทางเข้าพื้นที่ใหม่ระหว่างการบินตามปกติ

รุ่น `1.2.12` เคลื่อนยานภายในโลกด้วย mapped `Entity.setPos` หลังตรวจเส้นทางชนและพื้นที่ที่โหลดแล้วเหมือนเดิม การติดตามยานและตำแหน่งผู้ขี่ใช้ระบบ entity/passenger tick ตามปกติ ทั้งการบินและการลงจอดเลิกเรียก Bukkit teleport ทุก tick ไม่มีการแก้ไฟล์หรือ config ของ AeternumSeasons และไม่มีการกรอง packet ของฤดูกาล การวาร์ปผู้เล่นจริงยังส่ง event ตามเดิม

Paper ยังสร้าง `PlayerTeleportEvent` สาเหตุ `DISMOUNT` ตอนออกจากยาน รุ่นนี้ยกเลิกเฉพาะ event ของ Bedrock ที่เพิ่งลงจากไม้เท้าของตนเองใน tick เดียวกัน โดยต้องอยู่โลกเดิมและห่างจากตำแหน่งขี่ไม่เกิน 4 บล็อก ผู้เล่นออกจากยานแล้วและลงต่อด้วย Slow Falling ได้ การยกเลิกวาร์ปสั้นนี้ทำให้ arrival listener ที่รับเฉพาะวาร์ปสำเร็จไม่จัดคิวส่งฉากใหม่ การลงจากยานอื่นและการวาร์ปจริงยังเป็นไปตามเดิม

ทดสอบรุ่นเดิมบน Paper พบ event วาร์ปผู้ขี่ 15 ครั้งในช่วงบินสั้นเดียวกัน รุ่นใหม่ผ่าน 44 checks รวมการตรวจว่า event ระหว่างบินเหลือ 0 ครั้ง ผู้ขี่ตามยาน ตำแหน่งยานอยู่ในดัชนี chunk การลงของ Bedrock เอาผู้เล่นออกจากยานได้และไม่แจ้งวาร์ปสำเร็จให้ arrival listener และคำสั่งวาร์ปจริงยังถึง listener ข้อจำกัดคือใช้ mapped API ของ Paper 26.2 ที่ตรวจบนเซิร์ฟเวอร์ทดสอบแล้ว ยังต้องยืนยันว่าภาพกระพริบหายจาก Bedrock client จริง ตัวบันทึก `[StaffTrace]` ยังคงอยู่เพื่อเทียบ log รอบใหม่

### ผลจากการเล่นจริง 25 กันยายน 2026 เวลา 23:40

ผู้ใช้ยืนยันว่าเริ่มกระพริบเมื่อกด Shift ลง และหยุดเองหลังรอสักพัก Trace ของรุ่น `1.2.11` พบว่า:

- 23:40:33 และ 23:40:36: `DISMOUNT cancelled=false` หลังลง Paper/Geyser ต่างรายงาน `vehicle=none` ตำแหน่งตรงกัน และไม่มี teleport ค้าง
- 23:40:38–23:40:47: แม้ยืนบนพื้นและหยุดเดิน ยังได้รับ `ClientboundLevelChunkWithLightPacket` ประมาณ 40 ครั้งต่อวินาที โดยส่วนใหญ่เป็น chunk ที่เคยรับแล้ว ไม่มี player correction ต่อเนื่อง
- 23:40:51–23:40:52: การส่ง packet ชุดนั้นหยุดลง
- ใน AeternumSeasons 4.7 ที่ติดตั้ง `BiomePacketSender.send` คืน `false` เมื่อพบ Bedrock ใกล้ chunk เป้าหมาย ทำให้ `BiomeSpoofAdapter.flushClientRefreshQueue` ใช้ `World.refreshChunk` แทน biome packet การตั้งค่าใน `climate.yml` จำกัด fallback ที่ 2 chunks ต่อ 1 tick ตรงกับ 40 chunks ต่อวินาทีที่ตรวจพบ

ข้อมูลนี้ชี้ไปที่คิวรีเฟรชฉากที่สะสมขณะไม้เท้าเรียกการวาร์ปต่อเนื่อง และยังระบายต่อหลังลงจากไม้เท้า ต้องยืนยันผลบน Bedrock หลังแก้การเคลื่อนยาน ไม่ควรแก้ท่าถือหรือยกเลิกการลงจากรถเพิ่มจากอาการครั้งนี้

ผู้ใช้ยังพบอาการหลังใช้ `1.2.10` การดูคลิปเดิมทีละเฟรมพบว่ามีเฟรมที่พื้นและภูเขาหายไป แต่ HUD และไม้เท้ายังอยู่ จึงยังสรุปว่าเป็นแอนิเมชันไม้เท้าหรือการลงจากรถอย่างเดียวไม่ได้ รุ่น `1.2.11-flight-diagnostics` เป็นรุ่นเก็บข้อมูลเพิ่มเติม ยังไม่ได้ยืนยันว่าแก้อาการกระพริบได้

- บันทึก `[StaffTrace UUID]` ใน `logs/latest.log` สำหรับการขึ้นขี่ของผู้เล่น Bedrock ครั้งแรกหลังเปิดเซิร์ฟเวอร์ สูงสุด 120 วินาที และต่ออีกไม่เกิน 20 วินาทีหลังลงสำเร็จ
- เก็บตำแหน่งและรถของผู้เล่นฝั่ง Paper/Geyser, ศูนย์กลาง chunk, teleport ที่รอยืนยัน และจำนวน packet ของ chunk/biome/teleport/passenger ทุกหนึ่งวินาที ตัวบันทึกอ่านข้อมูลโดยไม่แก้ไขหรือส่ง packet เพิ่ม
- ปิดได้ด้วย `flying-staff.diagnostics.enabled: false` ตัวบันทึกถอด listener เมื่อจบช่วงบันทึก ผู้เล่นออก หรือปลั๊กอินปิด
- วิธีเก็บอาการ: เปลี่ยน JAR แล้ว restart ขี่และลงจากไม้เท้าภายในสองนาที จากนั้นอยู่ในเกมต่ออย่างน้อย 20 วินาที และจดเวลาที่เริ่มเห็นภาพกระพริบไว้เทียบกับ log
- Build ผ่าน และ Paper ทดสอบผ่าน 38 checks รวมการอ่าน API ของ Geyser ที่ติดตั้งจริง การนับ packet เฉพาะรถเป้าหมาย และการถอด listener หลังหยุด การตรวจนี้ไม่ได้จำลองภาพที่ client Bedrock เรนเดอร์


## Build บนเครื่องใหม่

ต้องมี Python 3, Java 25+ และ Maven ตามที่ระบุใน README จากโฟลเดอร์ราก ใช้คำสั่งนี้เพื่อสร้างแพ็ก ตรวจแพ็ก และสร้างปลั๊กอิน:

```sh
python3 advance-magic/tools/build_packs.py
python3 advance-magic/tests/check_packs.py
mvn -pl advance-magic -am package -DskipTests
cp advance-magic/target/advance-magic.jar dist/advance-magic.jar
```

การสร้างแพ็กอ่านต้นฉบับจาก `advance-magic/art/flying-staff/` และ `advance-magic/art/wands/` แล้วเขียนไฟล์แจกลง `advance-magic/dist/` ส่วน JAR ที่ build จะฝังแพ็กและ Geyser mapping เหล่านั้นไว้แล้ว

## ใช้งาน

ตั้งแต่ Evergarden `3.0.0-e2.9-flying-staff-spawn` ตัวกรอง `CreatureSpawnEvent` ยกเว้น Armor Stand ที่มีแท็กเจ้าของ `advance-magic:flying_staff_entity` ซึ่ง Advance Magic ใส่ก่อนเกิด entity เพื่อให้เรียกไม้เท้าในมิติ Evergarden ได้ ไม่ยกเว้นมอนสเตอร์หรือ Armor Stand ทั่วไป และไม่เปลี่ยน gamerule กันมอนสเตอร์เกิด

ผู้เล่นทั่วไปหาไม้เท้าได้จากกล่อง Whale, Hanging Garden, Observatory และ Evergarden Vault ตามอัตราด้านบน คราฟต์ไม่ได้ แอดมินให้ของทดสอบได้ด้วย `/magic givestaff [ชื่อผู้เล่น]` หรือเมนู `/magic items` สิทธิ์ใช้งาน `advance-magic.flying-staff` เปิดให้ผู้เล่นทั่วไป

ถือไม้เท้าคลิกขวาเพื่อเรียกข้างหน้า หรือเรียกตัวเดิมกลับมาเมื่อหายไปจากสายตา คลิกขวาที่ตัวไม้เท้าเพื่อขึ้นขี่ ย่องเมื่อต้องการลง Java ลงพร้อมไม้เท้า ส่วน Bedrock ลงได้ทันทีพร้อม Slow Falling ลงแล้วตีไม้เท้าเพื่อเก็บ ไอเทมเดิมไม่ถูกย้ายหรือดรอปซ้ำ เรียกได้หนึ่งตัวต่อคน ต้องยังมีไอเทมอยู่ในตัวจึงขี่ต่อได้

ขณะขี่ เดินหน้า/ถอย/ซ้าย/ขวาเพื่อเคลื่อนที่ กระโดดเพื่อสูงขึ้น มองลงเกิน 35° แล้วเดินหน้าเพื่อลงระดับ ปล่อยปุ่มเพื่อลอยค้าง ค่าเริ่มต้น `horizontal-speed: 0.486` และ `vertical-speed: 0.288` บล็อกต่อ tick เท่ากับความเร็ว Turbo เดิม ระบบค่อย ๆ ปรับความเร็วเมื่อเริ่มหรือหยุดกดปุ่ม และยกตัวข้ามพื้นต่างระดับไม่เกินหนึ่งบล็อกเมื่อบินต่ำเพื่อไม่ให้สะดุด ค่าเริ่มต้นเดิมใน config ของเซิร์ฟเวอร์จะถูกอัปเดตหนึ่งครั้ง ส่วนค่าที่แอดมินปรับเองจะคงเดิม

กด Sprint เพื่อเร่ง `turbo-multiplier: 4.0` เท่า (Java ใช้ Ctrl; Bedrock มือถือใช้ปุ่มวิ่ง) หรือคลิกขวาไม้เท้าในมือขณะขี่เพื่อเปิด/ปิด Turbo ค้าง ถ้าปุ่มใช้ไอเทมไม่ขึ้นบนมือถือ ใช้ `/magic turbo` แทนได้ ความเร็วปกติสูงสุดตามค่าเริ่มต้นคือ 0.486 บล็อกต่อ tick ในแนวราบ และ 0.288 ในแนวดิ่ง; Turbo คือ 1.944 และ 1.152 ตามลำดับ ระบบตรวจการชนตลอดเส้นทางเป็นช่วงสั้น ๆ เพื่อไม่ให้ความเร็วสูงพาทะลุบล็อก การขี่ปกติใช้มานาตาม `mana-per-second: 2.0` ส่วน Turbo ใช้คูณด้วย `turbo-mana-multiplier: 1.5` จึงใช้ 3 ต่อวินาทีตามค่าเริ่มต้น คิดค่าใช้มานาทุก tick ตามโหมดที่ใช้จริง การเร่งช่วงสั้น ๆ จึงเสียมานาเพิ่มตามเวลา และไม่มีค่าใช้มานาระหว่างจอด

มานาลด 2 ต่อวินาทีโดยค่าเริ่มต้น ระหว่างขี่หยุดฟื้นมานา เตือนเมื่อเหลือไม่เกิน 10 และลงจอดอัตโนมัติเมื่อหมด ถ้าลงจอดไม่ได้ใน 10 วินาทีหรือเข้าใกล้ขอบล่างโลก ระบบพากลับตำแหน่งที่เรียกไม้เท้า ตัวแสดงถูกลบเมื่อออกเกม ตาย ย้ายโลก ปิดปลั๊กอิน หรือไม่มีไอเทมในตัว

## Resource pack

`advance-magic/tools/build_packs.py` รวมโมเดลและแอนิเมชันเข้ากับแพ็กคทาเดิม โดยรักษา namespace/ไอเทมเดิมไว้ เพิ่ม Geyser mapping ในไฟล์เดียวกัน Bedrock pack version `2.1.0` ใช้ UUID ที่เปลี่ยนในรุ่น `1.9.0` ตัวแสดงผลบน Armor Stand ใช้ฐาน `carved_pumpkin` เพื่อให้ Java แสดง `item_model` แทนโมเดลเกราะ

มุมถือ Bedrock ของไม้เท้าบินและ Wand/Core of Restoration แปลงจากมุมและตำแหน่ง Java เป็นแกนของ attachable ที่ผูกกับมือ พร้อมเลื่อนจุดยึดให้ตรงโมเดล แพ็กก่อนหน้านี้นำค่าจาก Java ไปใช้ตรง ๆ จึงเห็นคทายื่นผิดมุมและต่ำกว่ามือ การแปลงอิง [เอกสาร attachable ของ Minecraft](https://learn.microsoft.com/en-us/minecraft/creator/documents/attachables?view=minecraft-bedrock-stable) และ [ตัวแปลง Rainbow ของ Geyser](https://github.com/GeyserMC/Rainbow)

คทาบินในแพ็ก `1.6.0` ผูก item slot กับ bone `anchor` ชั้นนอก แต่แอนิเมชันท่าถือขยับ `staff_root` ที่เป็นลูก รุ่น `1.7.0` ผูก item slot และแอนิเมชันบน `staff_root` โดยตรง แต่ภาพจากผู้ใช้ยืนถือหลังอัปเดตยังไม่ถูกต้อง

ภาพจาก Bedrock แพ็ก `1.7.0` ขณะยืนถือเฉย ๆ ยังแสดงคทาบังกลางล่างจอ โมเดล `flying_staff_held` เดิมลบ 8 จากพิกัด Y ของ Java ทั้งที่สูตรแปลงท่าถือคิดจากพิกัด Y เดิม รุ่น `1.8.0` คืนพิกัด Y ของโมเดลและจุดหมุน พร้อมกลับแกน X ให้ตรงกับตัวแปลง geometry ของ Rainbow และลดขนาด/เลื่อนท่าถือมุมมองบุคคลที่หนึ่งไปด้านขวาล่าง

ผู้ใช้รายงานว่าภาพหลังแพ็ก `1.8.0` ยังเหมือนเดิม ตรวจไฟล์จริงในเซิร์ฟเวอร์แล้วพบ JAR และ Geyser pack ตรงกับ `1.8.0` ทุกไบต์ โดย Geyser เริ่มหลังปลั๊กอินติดตั้งแพ็ก รุ่น `1.9.0` เปลี่ยน UUID ของแพ็กเพื่อให้ Bedrock ดาวน์โหลดชุดใหม่แน่นอน และย้าย cube ของโมเดลที่ถือทั้งหมดมาไว้บน bone ที่ผูกมือและแอนิเมชันท่าถือโดยตรง ผลใน client ยังต้องยืนยันจากภาพจริง

ภาพจากแพ็ก `1.9.0` ยืนยันว่าโมเดลที่ถือเปลี่ยนแล้ว แต่ในมุมบุคคลที่หนึ่งคทาใหญ่และยื่นข้ามจอ มุมบุคคลที่สามอยู่แถวเท้า รุ่น `2.0.0` หมุนและย่อ geometry ของคทาที่ถือให้ใช้แกนและจุดหมุนแบบ [Trident ของ Mojang](https://github.com/Mojang/bedrock-samples/blob/main/resource_pack/models/entity/trident.geo.json) แล้วใช้ [ท่าถือ Trident](https://github.com/Mojang/bedrock-samples/blob/main/resource_pack/animations/trident.animation.json) เป็นต้นแบบแยกตามมุมกล้อง Java และโมเดลคทาที่เรียกออกมาไม่เปลี่ยน

ภาพจากแพ็ก `2.0.0` เวลา 22:30 ยืนยันว่าหัวคริสตัลชี้ลงและหลุดขอบล่างจอ ขณะที่ท้ายด้ามชี้ขึ้น จุดหมุน Y 24 อยู่ในหัวคริสตัล ส่วนกลางด้ามจับสีน้ำเงินอยู่ที่ Y 11.05205 รุ่น `2.1.0` หมุน geometry ที่ถือ 180 องศารอบ X แล้วเลื่อนกลางด้ามจับไปที่ pivot `[0,24,0]` โดยใช้ `(x,y,z) -> (x,35.05205-y,-z)` ท่าบุคคลที่หนึ่งคงเดิมเพื่อกลับหัวกับท้ายโดยอิงภาพจริง ส่วนบุคคลที่สามชดเชยมุม X จาก 97 เป็น -83 องศาเพื่อรักษาทิศเดิมหลังกลับ geometry พร้อมย้ายจุดจับไปที่ด้าม เพิ่ม locator `grip` เพื่อระบุจุดยึดให้ชัดเจน การจัดท่านี้ยังต้องยืนยันจาก Bedrock จริงทั้งสองมุมกล้อง

คลิป Bedrock วันที่ 25 กันยายน เวลา 22:41 แสดงภาพคทากระพริบระหว่างบิน ฝั่งเซิร์ฟเวอร์เคยเปลี่ยนเฟรมของหมวก Armor Stand ทุก 4 ticks ขณะบิน และทุก 2/8 ticks ในสถานะอื่น ทั้งที่ attachable ของ Bedrock มีแอนิเมชันของตัวเอง รุ่น `1.2.9` ตรวจผู้เล่น Bedrock ผ่าน Geyser/Floodgate แล้วส่งหมวกเฉพาะตอนเปลี่ยนสถานะ ให้แอนิเมชันบน client เล่นต่อเนื่อง Java ยังรับเฟรมหมวกตามเดิม แพ็ก Bedrock ยังคงรุ่น `2.1.0` เพราะไม่มีการแก้ไฟล์ภาพ ต้องยืนยันว่าภาพกระพริบหายจาก client จริง

ผู้ใช้รายงานว่า Bedrock ยังเห็นภาพกระพริบตอนลงจากไม้เท้าหลังใช้ `1.2.9` รุ่น `1.2.10` แก้เส้นทางลงจากที่สูง: Bedrock ลงได้ทันทีพร้อม Slow Falling และไม้เท้าลงพื้นแยกต่างหาก เพื่อเลี่ยงการยกเลิก `EntityDismountEvent` ที่ทำให้ client คิดว่าลงแล้วแต่เซิร์ฟเวอร์ยังให้ขี่อยู่ เพิ่มระยะรอลงจอดจาก 200 เป็น 1200 ticks เพราะการลดระดับ 0.1 บล็อกต่อ tick ใช้เกิน 200 ticks เมื่อบินสูง Java ยังคงลงพร้อมไม้เท้า และไม่เริ่มนับเวลาลงจอดใหม่เมื่อผู้เล่นกดย่องซ้ำ ตอนลงใกล้พื้น Bedrock จะเปลี่ยนภาพไม้เท้าเป็นสถานะ idle หลังผู้เล่นลงจากรถแล้วสอง ticks เพื่อไม่สลับภาพขณะที่ยังนั่งอยู่ การแก้ปัญหานี้ยังต้องตรวจใน Bedrock จริง

Java เปิด bundled HTTP pack host บน TCP 8187 เป็นค่าเริ่มต้น เพราะ URL ของแพ็กเก่าที่เคยตรึงไว้ยังไม่มีไม้เท้า ระบบย้าย config ที่ชี้ URL ทางการรุ่นเก่ามาใช้ bundled host อัตโนมัติ และคำนวณ SHA-1 จาก ZIP ที่อยู่ใน JAR จริง URL ที่แอดมินกำหนดเองยังคงเดิม: ต้องอัปโหลด ZIP รุ่นใหม่นี้และตั้ง SHA-1 ให้ตรงเอง มิฉะนั้น Java จะเห็นภาพเก่า

ผู้เล่น Java ต้องเข้าถึงพอร์ต 8187 ของเซิร์ฟเวอร์ หากใช้ proxy/SRV ให้ตั้ง `resource-pack.host.public-host` หรือ `resource-pack.host.public-url` ให้ชี้มาที่ host นี้ ตรวจผลด้วย `/magic pack` และ `/magic pack resend` ถ้า host ไม่เริ่ม ระบบจะแจ้งใน log และไม่เสนอ ZIP รุ่นเก่าผิด hash

Geyser-Spigot บนเครื่องเดียวกันจะรับ Bedrock pack และ mapping จาก JAR ในช่วง `onLoad` ก่อน Geyser เริ่ม หาก Geyser แยกเครื่อง ให้คัดลอก `advance-magic/dist/advance-magic-bedrock.mcpack` ไป `packs/` และ `advance-magic/dist/geyser-mappings.json` ไป `custom_mappings/advance-magic.json` แล้วรีสตาร์ต Geyser/ให้ผู้เล่นรับแพ็กใหม่

## `allow-flight=false` และ anti-cheat

ไม้เท้าใช้ Armor Stand ที่เซิร์ฟเวอร์ขยับทีละระยะโดยรักษา passenger ไว้ ผู้เล่นเป็นผู้โดยสาร ระบบไม่สั่ง `setAllowFlight(true)` หรือ `setFlying(true)` จึงใช้กับ `server.properties` ที่ตั้ง `allow-flight=false` ได้ตามที่ทดสอบบน Paper การเคลื่อนที่ถูกจำกัดความเร็วและตรวจทางโล่งก่อนขยับ ความลื่นของภาพยังต้องตรวจจาก client จริง

ขณะขี่เท่านั้น ระบบให้ permission attachment ตาม `flying-staff.anticheat.bypass-permissions` ถ้าพบปลั๊กอินชื่อนั้น ติดตั้งค่าเริ่มต้นสำหรับ GrimAC (`grim.disabled`) และ NoCheatPlus (`nocheatplus.checks.moving.vehicle`) ถอด attachment ตอนลงจอด ออกจากเกม หรือลบไม้เท้า การให้สิทธิ์ Grim ปิดการตรวจของ Grim ทั้งหมดชั่วคราวระหว่างขี่ จึงควรใช้กับผู้เล่นที่ถือไม้เท้าอย่างถูกต้องเท่านั้น [Grim permissions](https://github.com/GrimAnticheat/Grim/wiki/Permissions), [NoCheatPlus vehicle permission](https://github.com/NoCheatPlus/NoCheatPlus/blob/master/NCPPlugin/src/main/resources/plugin.yml)

ปลั๊กอิน anti-cheat อื่นเติมรายการใน config เป็น `ชื่อปลั๊กอิน: [ชื่อ.permission]` ได้เมื่อมีเอกสารยืนยัน วิธีนี้ไม่ได้สั่ง anti-cheat ทุกยี่ห้อให้ bypass อัตโนมัติ บางตัว cache permission หรือมี API เฉพาะ ตัวอย่าง Matrix มี API `reloadPermissionCache` และ `tempBypass` แยก จึงต้องทำ adapter เฉพาะถ้าเซิร์ฟเวอร์นั้นใช้ Matrix [Matrix API](https://matrix.rip/docs/developer/api)

## ตรวจแล้วและยังต้องตรวจ

- Maven build ผ่าน; ตรวจโครงสร้าง ZIP, JSON, model, mappings, PNG, Bedrock manifest version, SHA-1 และไฟล์ที่ฝังใน JAR
- รุ่น `1.2.2-flying-staff` build เป็น JAR ผ่านแล้ว; การปรับความเร็ว 4 เท่าและการยกตัวข้ามพื้นต่างระดับยังต้องลองขี่บนเซิร์ฟเวอร์จริง
- Paper 26.2 บนเซิร์ฟเวอร์ทดสอบแยกที่ตั้ง `allow-flight=false` ผ่าน 32 การตรวจ: เรียกและขึ้นขี่จาก Bukkit event จริง, ตรวจไอเทมแสดงผลบนหัว Armor Stand, ส่งปุ่มเดินหน้าและ Sprint ผ่าน packet แล้ววัดความเร็ว, ใช้ `/magic turbo` เพื่อเร่งค้าง, อัปเดตค่า config เดิมโดยรักษาค่าที่ปรับเอง, มานาลดสุทธิ, ไม่มีสำเนาไม้เท้า, พาหนะถูกเก็บ, ลงจอดเมื่อมานาหมด และ HTTP แจก ZIP ตรงกับ SHA-1
- Geyser-Spigot 2.11.3 บนเซิร์ฟเวอร์ทดสอบรันพร้อม JAR รุ่น `1.2.1-flying-staff` และแพ็ก Bedrock `1.5.0` ได้ จดทะเบียน custom items รวม 41 รายการ; ไฟล์ที่ติดตั้งใน `packs/` และ `custom_mappings/` ตรงกับไฟล์ที่ build ทุกบิต และ Paper ผ่าน 32 การตรวจในรอบเดียวกัน
- ตรวจ bytecode ของไคลเอนต์ Java 26.2: `LivingEntityRenderer` ส่งไอเทมที่ไม่เข้าเงื่อนไข armor layer ไป `ItemModelResolver` ด้วยบริบท `HEAD`; `CustomHeadLayer` วาดผลลัพธ์นั้นบนหัวโมเดล ส่วน Geyser 2.11.3 แปลงจอยสัมผัสเป็นปุ่มเดินของ Java และ `SPRINT_DOWN` เป็นปุ่ม Sprint ใน packet อินพุต การตรวจนี้ยืนยันเส้นทางโค้ด แต่ยังไม่ใช่การเห็นภาพในเกมจริง
- ใช้ตัวอ่านโมเดลจริงจากไคลเอนต์ Minecraft Java 26.2 ตรวจ Java pack ที่ build แล้ว: `CuboidModel.fromStream` รับโมเดลไม้เท้า 30 ไฟล์ และ `ClientItem.CODEC` รับนิยามไอเท็มทั้ง 5 สถานะ ไม่พบข้อผิดพลาดในขั้นตอน parse; ยังไม่ได้ยืนยันภาพที่เรนเดอร์บนหน้าจอ
- ทดสอบการให้และถอน permission กับปลั๊กอินจำลองชื่อ GrimAC; ยังไม่ได้ทดสอบกับ GrimAC หรือ NoCheatPlus จริง
- ยังไม่ได้ต่อ Java/Bedrock client เพื่อดูท่านั่ง การเลี้ยว การวางบล็อกบนไม้เท้า ภาพ Geyser หรือพาร์ติเคิล Bedrock จึงต้องทดสอบบนเซิร์ฟเวอร์ staging ก่อนเปิดให้ผู้เล่น

ไม่เปลี่ยน `server.properties`, ไฟล์ anti-cheat หรือเซิร์ฟเวอร์จริงของผู้ใช้ในงานนี้
