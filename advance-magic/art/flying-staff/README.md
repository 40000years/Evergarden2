...
Bedrock geometry `flying_staff_display` ยึด bone `head`; `staff_root` อยู่ที่ `[0,24,0]` ส่วน `flying_staff_held` ผูก item slot และแอนิเมชันท่าถือบน `staff_root` ตัวเดียวกันที่ pivot `[0,8,0]` โดยแปลงพิกัดมาจากรูปทรง Java: X กลับด้าน, Y ใช้ค่า Java เดิม และ Z ลบ 8 มุมมองบุคคลที่หนึ่งของ Bedrock ลดขนาดและเลื่อนไปด้านขวาล่างเพื่อไม่บังกลางจอ
