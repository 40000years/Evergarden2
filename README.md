...
Install `advance-magic/target/advance-magic.jar` and `evergarden/target/evergarden.jar` only on a separate test server. These JARs use the existing plugin names and commands, so do not install alongside their release versions. The default world name is `evergarden2`. Existing server configs are not overwritten by a JAR update; use fresh plugin data folders or review the two `src/main/resources/config.yml` files.
