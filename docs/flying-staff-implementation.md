...
โค้ดอยู่ใน Advance Magic รุ่น `1.2.0-flying-staff` แพ็กทั้งสองฝั่งอยู่ใน JAR และ `advance-magic/dist/` ไฟล์แยกใน `advance-magic/art/flying-staff/dist/` เป็นงาน preview ไม่ใช่แพ็กที่เซิร์ฟเวอร์แจก
...
ขณะขี่ เดินหน้า/ถอย/ซ้าย/ขวาเพื่อเคลื่อนที่ กระโดดเพื่อสูงขึ้น มองลงเกิน 35° แล้วเดินหน้าเพื่อลงระดับ ปล่อยปุ่มเพื่อลอยค้าง ค่าเริ่มต้น `horizontal-speed: 0.18` และ `vertical-speed: 0.12` บล็อกต่อ tick ความเร็วนี้จำกัดใน config เหลือไม่เกิน 0.35 และ 0.25 ตามลำดับเพื่อไม่ให้เคลื่อนที่เร็วเกินไป
...
`advance-magic/tools/build_packs.py` รวมโมเดลและแอนิเมชันเข้ากับแพ็กคทาเดิม โดยรักษา namespace/ไอเทมเดิมไว้ เพิ่ม Geyser mapping ในไฟล์เดียวกัน Bedrock pack version `1.4.0` ขยับจาก `1.3.0` เพื่อบังคับ client รับของใหม่
...
- Paper 26.2 บนเซิร์ฟเวอร์ทดสอบแยกที่ตั้ง `allow-flight=false` ผ่าน 24 การตรวจ: เรียกและขึ้นขี่จาก Bukkit event จริง, ส่งปุ่มเดินหน้า/กระโดดผ่าน packet ทดสอบแล้วพาหนะขยับ, มานาลดสุทธิ, ไม่มีสำเนาไม้เท้า, พาหนะถูกเก็บ, ลงจอดเมื่อมานาหมด และ HTTP แจก ZIP ตรงกับ SHA-1
