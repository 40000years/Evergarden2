...
Bedrock ใช้ `animations/flying_staff.animation.json` มี 6 clips: ท่าถือ first/third person และ 4 สถานะข้างต้น attachable แต่ละตัวเรียก clip ของตัวเอง `summon` และ `dismiss` ค้างเฟรมสุดท้าย; ปลั๊กอินเปลี่ยนหมวกเมื่อเปลี่ยนสถานะและลบภาพตามเวลา โดยไม่เปลี่ยนเฟรมหมวกซ้ำระหว่างแอนิเมชัน เพื่อไม่ให้ Bedrock รีเซ็ตภาพถี่ ๆ
...
Bedrock geometry `flying_staff_display` ยึด bone `head`; `staff_root` อยู่ที่ `[0,24,0]` ส่วน `flying_staff_held` ผูก item slot, แอนิเมชันท่าถือ และ cube ทั้งหมดไว้บน bone เดียว โมเดลที่ถือใช้แกนด้ามตาม Y และ pivot `[0,24,0]` ตรงกลางด้ามจับสีน้ำเงิน หัวคริสตัลอยู่ทาง Y ต่ำกว่าจุดจับ ส่วนท้ายอยู่ทาง Y สูงกว่า เพื่อให้หัวชี้ขึ้นภายใต้ท่าบุคคลที่หนึ่งที่ใช้อยู่ ท่าบุคคลที่สามชดเชยการกลับแกนด้วยมุม X -83 องศา Locator `grip` ระบุจุดจับบนแกนด้าม ท่าถือเริ่มจาก [แอนิเมชัน Trident ของ Mojang](https://github.com/Mojang/bedrock-samples/blob/main/resource_pack/animations/trident.animation.json) แล้วแก้ตำแหน่งด้ามและทิศจากภาพ Bedrock ของผู้ใช้ โมเดลคทาที่เรียกออกมาบน Armor Stand ยังคงแยกจากโมเดลที่ถือ
